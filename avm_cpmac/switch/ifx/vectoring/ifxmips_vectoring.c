...
static void proc_output(struct seq_file *m, void *data __maybe_unused);
static int proc_input(char *line, void *data __maybe_unused);
...
static int mei_dsm_cb_func(unsigned int *p_error_vector)
{
    ...
    while ( (skb = skb_list) != NULL ) {
        ...
        /* highest globally known priority as defined by dsld.c */
        skb->priority = TC_H_MAKE(10,1);
...
static int __init vectoring_init(void)
{
    ...
    add_simple_proc_file( "vectoring", proc_input, proc_output, NULL);
...
static void __exit vectoring_exit(void)
{
    ...
    remove_simple_proc_file("vectoring");
}
