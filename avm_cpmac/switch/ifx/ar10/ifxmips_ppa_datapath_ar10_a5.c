 ...
 * ####################################
 */
static unsigned int port_cell_rate_up[2] = { 3200, 3200 }; /*  Maximum TX cell rate for ports                  */

static unsigned int qsb_tau = 1; /*  QSB cell delay variation due to concurrency     */
static unsigned int qsb_srvm = 0x0F; /*  QSB scheduler burst length                      */
static unsigned int qsb_tstep = 4; /*  QSB time step, all legal values are 1, 2, 4     */

static unsigned int write_descriptor_delay = 0x20; /*  Write descriptor delay                          */
...
#define AVM_PPE_CLOCK_ATA						400
#define AVM_PPE_CLOCK_DSL						250
 ...
 */
#define AR10_CGU                                 (KSEG1 | 0x1F103000)
...
static int ifx_ppa_eth_qos_hard_start_xmit_a5(struct sk_buff *, struct net_device *);
...
static const struct atmdev_ops g_ppe_atm_ops = { owner: THIS_MODULE,
  open: ppe_open,
  close: ppe_close,
  ioctl: ppe_ioctl,
  send: ppe_send,
...
static int ppe_ioctl(struct atm_dev *dev __attribute__((unused)),
	...
	dbg("cmd=%#x", icmd);
	if (_IOC_TYPE(icmd) != PPE_ATM_IOC_MAGIC || _IOC_NR(icmd) >= PPE_ATM_IOC_MAXNR)
		return -ENOTTY;

	if (_IOC_DIR(icmd) & _IOC_READ)
		ret = !access_ok(VERIFY_WRITE, arg, _IOC_SIZE(icmd));
	else if (_IOC_DIR(icmd) & _IOC_WRITE)
		ret = !access_ok(VERIFY_READ, arg, _IOC_SIZE(icmd));
	if (ret)
		return -EFAULT;
...
static int ppe_open(struct atm_vcc *vcc) {
	...
	if ((vcc->qos.txtp.traffic_class == ATM_CBR
			&& (int)vcc->qos.txtp.max_pcr
					> (int)(port->tx_max_cell_rate - port->tx_used_cell_rate))
			|| (int)(vcc->qos.txtp.traffic_class == ATM_VBR_RT
					&& (int)vcc->qos.txtp.max_pcr
							> (int)(port->tx_max_cell_rate - port->tx_used_cell_rate))
	...
	for (sw_tx_queue = 0; sw_tx_queue < g_atm_priv_data.max_sw_tx_queues;
			sw_tx_queue++)
...
static void ppe_close(struct atm_vcc *vcc) {
	...
	union {
		volatile const struct atm_connection *ro;
		struct atm_connection *rw;
	} connection;
	unsigned int i;
	...
	connection.ro = &g_atm_priv_data.connection[conn];
	port = &g_atm_priv_data.port[connection.ro->port];
	...
	/*  release connection  */
	g_atm_priv_data.connection_table &= ~(1 << conn);
	g_atm_priv_data.sw_tx_queue_table &= ~(connection.ro->sw_tx_queue_table);
	memset(connection.rw, 0, sizeof(*connection.rw));
...
static int ppe_send(struct atm_vcc *vcc, struct sk_buff *skb) {
	...
	if ((g_dbg_datapath & DBG_ENABLE_MASK_MAC_SWAP)) {
		...
		if (!(p[0] & 0x01)) { //  bypass broadcast/multicast
			...
			dma_cache_wback((unsigned long)skb->data,
        ...
        case ETH_MAP_PKT_PRIO_TO_Q: 
            {
                ...
                if (cmd.pkt_prio < 0 || cmd.pkt_prio >= (int)NUM_ENTITY(g_eth_prio_queue_map))return -EINVAL;
...
static int sw_tx_queue_del(int conn) {
	...
	if (g_atm_priv_data.connection[conn].sw_tx_queue_table == (1U << rem_sw_tx_queue)) {
		ret = -EIO;
		goto SW_TX_QUEUE_DEL_EXIT;
	}

	new_sw_tx_queue_table = g_atm_priv_data.connection[conn].sw_tx_queue_table
			& ~(1 << rem_sw_tx_queue);
...
static int sw_tx_prio_to_queue(int conn, int prio, int queue,
		int *p_num_of_virt_queue) {
	int virt_queue_to_sw_tx_queue_map[8] = { 0 };
	unsigned int num_of_virt_queue;
	u32 sw_tx_queue_table;
	int sw_tx_queue;
	...
	if (prio < 0 || queue < 0 || queue >= (int)num_of_virt_queue) {
		if (!in_interrupt())
			up(&g_sem);
		return -EINVAL;
	}

	g_atm_priv_data.connection[conn].prio_queue_map[prio] =
			virt_queue_to_sw_tx_queue_map[queue];
...
\*------------------------------------------------------------------------------------------*/
void ifx_ppa_atm_hard_start_xmit(struct sk_buff *skb, struct flag_header *fheader) {
    unsigned long sys_flag;
    volatile struct tx_descriptor *desc;
    struct tx_descriptor reg_desc;
    int byteoff;
...
/*------------------------------------------------------------------------------------------*\
\*------------------------------------------------------------------------------------------*/
void local_dump_skb(struct sk_buff *skb, u32 len, const char *title) {
    unsigned int i;
...
static INLINE int handle_skb_a5(struct sk_buff *skb)
{
...
#endif
    unsigned int exp_len, hw_dev_id;

	dbg_trace_ppa_data("[%s] s\n", __FUNCTION__);

#if defined(CONFIG_AVMNET_DEBUG) 
	dump_skb(skb, DUMP_SKB_LEN, "handle_skb_a5 raw data", 0, 0, 0, dump_enforce);
#endif
    ...
    */
    if ( ! fheader->acc_done ) {
        ...
        */
        ...
        if(exp_len != skb->len ){
            ...
            local_dump_skb( skb, 64, "handle_skb_e5" );
            dev_kfree_skb_any(skb);
            return 0;
        }
    }
	...
	dump_skb(skb, DUMP_SKB_LEN, "handle_skb_a5 packet data", fheader->src_itf, 0, 0, dump_enforce);
#endif
	...
	if (fheader->acc_done && fheader->dest_list) //  acc_done == 1 && dest_list != 0
		...
		} else if ((fheader->dest_list & (1 << 7))) //  7 - DSL
		{
			...
			for (dst_id = 3; dst_id < 3 + MAX_PPA_PUSHTHROUGH_DEVICES; dst_id++) {
				if ((fheader->dest_list & (1 << dst_id))) //  3...6 (einschliesslich 3 u. 6)
				{
					...
					if ( ifx_lookup_routing_entry && ppa_virt_tx_devs[ if_id ] ){
						...
						dbg_trace_ppa_data("Achtung ungesteter Code!\n");

    					dbg_trace_ppa_data("ACC-Done: ifid=%d, avm_rout_index=%d, is_mc=%d, is_lan=%d",
    					        if_id,
    					...
    					ppa_virt_tx_devs[ if_id ]->hw_pkt_tx ++;
                      	...
                      	avm_pa_session = ifx_lookup_routing_entry(fheader->avm_rout_index, fheader->avm_is_lan, fheader->avm_is_mc );
				...
				//else
				{
					...
					if (WRX_QUEUE_CONFIG(conn)->mpoa_mode) {
						...
						case 2: //  PPPoA
							...
							DUBIOUS_ASSERT(
									(int)fheader->ip_inner_offset - 6 >= 0 || skb_headroom(skb) >= 6 - fheader->ip_inner_offset,
						...
						case 3: //  IPoA oder RAW-Packet fuer kdsld-probe
							if ( fheader->ip_outer_offset ) {
								...
								dbg_trace_ppa_data("seems to be raw packet, default_raw_offset=%d (current_ip inner_offset = %d, outer_offset=%d)", 
                                                   default_raw_offset, fheader->ip_inner_offset, fheader->ip_outer_offset);
							}
							break;
						}
					}
					dump_skb(skb, DUMP_SKB_LEN, "dma_rx_int_handler AAL5 encapsulated", fheader->src_itf, 0, 0, dump_enforce);
				}
...
/*------------------------------------------------------------------------------------------*\
\*------------------------------------------------------------------------------------------*/
static int pseudo_dma_int_handler(struct dma_device_info *dma_dev, int status, int chan_no) {
    int ret = 0;
    struct sk_buff *skb = NULL;
    u8 *buf;
    ...
    {
    case RCV_INT:
        dma_device_read(dma_dev, &buf, (void **)&skb, chan_no);
        if ( skb )
            dev_kfree_skb_any(skb);
    }
...
static INLINE void reset_ppe(void) {
    ...
    dma_device->intr_handler    = pseudo_dma_int_handler;
    dma_device->tx_burst_len    = 8;
    dma_device->rx_burst_len    = 8;
    dma_device->port_packet_drop_enable = 1;
...
static INLINE void start_cpu_port(void) {
	...
	*PCE_PCTRL_REG(6, 0) |= 0x07; //  clear_dplus
	...
	disable_cpuport_flow_control(); // has been in enable_fdma_sdma before
}
...
//        *PCE_PCTRL_REG(i, 2) |= 3;  //  highest class, mapped to RX channel 0
	...
	*PMAC_RX_IPG_REG = 0x8F;
	...
	//IFX_AR10_Switch_PCE_Micro_Code_Int();
 ...
 *             else: Error Code
 */
static INLINE int pp32_specific_init(int fwcode __attribute__((unused)), void *data __attribute__((unused)))
{
    return 0;
}
...
static INLINE int pp32_start(int fwcode)
{
    ...
    /*  download firmware   */
    ret = pp32_download_code(ar10_a5_binary_code, sizeof(ar10_a5_binary_code) / sizeof(*ar10_a5_binary_code), ar10_a5_binary_data, sizeof(ar10_a5_binary_data) / sizeof(*ar10_a5_binary_data));
    if ( ret )
        return ret;
...
static INLINE int init_local_variables(void) {
...

	{
		...
		unsigned int tx_num_q;
		unsigned int q_step, q_accum, p_step;
...
//	gen_mode_cfg1.ipv6_rout_num = g_ipv6_acc_en ? 2 : 0;
    ...
    *CFG_QID2CLASS_MAP(1) = 0;
    ...
    {
        printk(KERN_ERR "[%s] write g_eth_prio_queue_map[%d] = %d to hw\n", __func__, j,g_eth_prio_queue_map[j]);
...
static INLINE int alloc_dma(void) {
	...
	if (g_eth_wan_mode == 0) { //  DSL WAN
		...
		pre_alloc_desc_total_num += DMA_RX_CH1_DESC_NUM + CPU_TO_WAN_TX_DESC_NUM
				...
				/* + CPU_TO_WAN_SWAP_DESC_NUM , das war falsch, denn es ist der gleiche Speicherbereich, wie CPU_TO_WAN_TX_DESC */
				+ DMA_TX_CH1_DESC_NUM;
    }

	skb_pool = kmalloc(pre_alloc_desc_total_num * sizeof(*skb_pool), GFP_KERNEL);
	if (skb_pool == NULL) {
		ret = -ENOMEM;
    ...
    // VORSICHT: Der folgende Code ist falsch! - AR9 / AR10 benutzen den Gleichen Speicher fuer
    // CPU_TO_WAN_SWAP_DESC_BASE und DSL_WAN_TX_DESC_BASE(0)
    // Somit darf nur einmal Alloc bzw. Free aufgerufen werden
#if 0
	if (g_wanqos_en) {
...
static INLINE void free_dma(void) {
    ...
    if ( g_eth_wan_mode == 0 || g_wanqos_en )
    {
        ...
        printk(KERN_ERR "[%s] free cpu -> shared mem \n", __func__);
        ...
        // should be cleared here? not handled by DMA-HW
        printk(KERN_ERR "[%s] free dsl wan tx \n", __func__);
    ...
    if ( g_wanqos_en )
    {
        // should be cleared here? used in MBOX irq
        printk(KERN_ERR "[%s] free wan swap \n", __func__);
    ...
    if ( g_eth_wan_mode == 0 )
    {
        // should be cleared here? used in MBOX (OAM) irq
        printk(KERN_ERR "[%s] free mbox oam0 \n", __func__);
        ...
        // should be cleared here? used in MBOX (OAM) irq
        printk(KERN_ERR "[%s] free mbox oam1 \n", __func__);
...
#if 0
	// PPA -> DSL cleard by DMA unregister (is real dma rx-chan with no_cpu_data)
    printk(KERN_ERR "[%s] free ppa -> dsl \n", __func__);
...
#if !defined(ENABLE_P2P_COPY_WITHOUT_DESC) || !ENABLE_P2P_COPY_WITHOUT_DESC
	// RX2 -> TX0: (PPA -> Switch(port6)) cleard by DMA unregister (is real dma rx-chan with no_cpu_data)
    printk(KERN_ERR "[%s] free rx2 -> tx0 \n", __func__);
    ...
    // DSL -> Switch(port11) not cleared by DMA unregister (is not a real dma rx-chan)
    if ( g_wanqos_en )
    {
        printk(KERN_ERR "[%s] free dsl -> switch \n", __func__);
...
int atm_showtime_enter(struct port_cell_info *port_cell, void *xdata_addr) {
	...
	dbg(
			"enter showtime, cell rate: 0 - %d, 1 - %d, xdata addr: 0x%08x",
...
static int ppe_clk_change(unsigned int arg, unsigned int flags) {
	...
	*AR10_CGU_CLKFSR  = (*AR10_CGU_CLKFSR & ~(0x07 << 16)) | ppe_cfg_bits;
	local_irq_restore(sys_flag);

    udelay(1000);

	printk(KERN_ERR "clk=%d, cfg_bits=%#x \n", arg, ppe_cfg_bits );
...
/*------------------------------------------------------------------------------------------*\
\*------------------------------------------------------------------------------------------*/
void avmnet_swi_ar10_a5_register_xmit (void) {
	unsigned int i;

	pr_err("[%s] setup new xmit function \n", __func__);
	...
	for ( i = 0; i < num_registered_eth_dev; i++ ) {
		...
		if (avm_ata_dev &&
		    (strncmp(dev->name, avm_ata_dev, NETDEV_NAME_LEN) == 0)) {
			ops.rw->ndo_start_xmit =
				&ifx_ppa_eth_qos_hard_start_xmit_a5;
			dev->features &= ~(NETIF_F_SG | NETIF_F_IP_CSUM);
		} else {
			ops.rw->ndo_start_xmit = &ifx_ppa_eth_hard_start_xmit_a5;
		}

		pr_err("[%s] xmit %s = %pF \n",
		       __func__, dev->name, ops.rw->ndo_start_xmit);

		ops.rw->ndo_tx_timeout = &ifx_ppa_eth_tx_timeout_a5;
		ops.rw->ndo_do_ioctl = &ifx_ppa_eth_ioctl_a5;
	}
}
...
static int ppe_a5_init(void) {
    ...
    //TODO check mailbox irq

    // tell driver to suspend packet flow on all devices
    if(avmnet_hw_config_entry->config->suspend){
        int wait_cycles = 0;
        while( avmnet_hw_config_entry->config->suspend(avmnet_hw_config_entry->config, NULL)){
            wait_cycles++;
            schedule();
        }
        if (wait_cycles)
            printk(KERN_ERR "[%s] had to wait %d cycles to get switch_lock\n", __func__, wait_cycles);
    }else{
	...
	for (i = 0; i < ATM_PORT_NUMBER; i++) {
		g_atm_priv_data.port[i].dev =
			ifx_atm_dev_register("ifx_atm", NULL, &g_ppe_atm_ops,
					     -1, 0UL);
    ...
    ret = request_irq(IFX_PPE_MBOX_INT0, mailbox0_irq_handler, IRQF_DISABLED, "a5_mailbox0_isr", NULL);
    if ( ret )
    {
        if ( ret == -EBUSY )
	...
	*MBOX_IGU1_IER = g_mailbox_signal_mask;
    ...
    if(avmnet_hw_config_entry->config->resume){
        while( avmnet_hw_config_entry->config->resume(avmnet_hw_config_entry->config, NULL) )
            schedule();
    }else{
