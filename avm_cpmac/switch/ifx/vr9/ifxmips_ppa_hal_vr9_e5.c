...
/*@}*/
...
#if defined(CONFIG_AVMNET_DEBUG_HAL) 
  ...
  module_param(ifx_ppa_hal_dbg_enable, uint, S_IRUSR | S_IWUSR);
...
static void add_tunnel_6rd_header(volatile struct iphdr *iph, __be32 saddr, __be32 daddr );
...
static uint32_t set_qos_port_id(void)
{
    ...
    if( mixed_mode ) {
        //not support QOS in mixed mode
        qos_queue_portid = -1; //invalid
        res = IFX_FAILURE;
    }

    if( wan_itf & IFX_PPA_DEST_LIST_ETH0 )
        qos_queue_portid = PPA_WAN_QOS_ETH0;
    else if( wan_itf & IFX_PPA_DEST_LIST_ETH1 )
        qos_queue_portid = PPA_WAN_QOS_ETH1;
    else if ( wan_itf & IFX_PPA_DEST_LIST_ATM )
        qos_queue_portid = PPA_WAN_QOS_PTM0;
    else {
        qos_queue_portid = -1; //invalid
        res = IFX_FAILURE;
    }
...
static const uint8_t g_capwap_entries = 5;
...
*/
    ...
    if ( ifname ){
        strncpy(ifname, str_ifname[port], PPA_IF_NAME_SIZE - 1 );
        ifname[PPA_IF_NAME_SIZE - 1] = 0 ;
    }
...
void set_bridging_cfg(uint32_t entry_num __attribute__((unused)),
                      uint32_t br_to_src_port_mask __attribute__((unused)), uint32_t br_to_src_port_en __attribute__((unused)),
                      uint32_t f_dest_vlan_en __attribute__((unused)),
                      uint32_t f_src_vlan_en __attribute__((unused)),
                      uint32_t f_mac_change_drop __attribute__((unused)),
...
/*!
  ...
  \return no return value
 */
void set_if_wfq(uint32_t if_wfq __attribute__((unused)),
                uint32_t if_no __attribute__((unused)))
{
}
...
/*!
  ...
  \return no return value
 */
void set_dplus_wfq(uint32_t wfq __attribute__((unused)))
{
}
...
/*!
  ...
  \return no return value
 */
void set_fastpath_wfq(uint32_t wfq __attribute__((unused)))
{
}
...
/*!
  ...
  \return no return value
 */
void set_default_dest_list(uint32_t uc_dest_list __attribute__((unused)),
                           uint32_t mc_dest_list __attribute__((unused)),
                           uint32_t if_no __attribute__((unused)))
{
}

void set_bridge_if_vlan_config(uint32_t if_no __attribute__((unused)),
                               uint32_t f_eg_vlan_insert __attribute__((unused)),
                               uint32_t f_eg_vlan_remove __attribute__((unused)),
                               uint32_t f_ig_vlan_aware __attribute__((unused)),
                               uint32_t f_ig_src_ip_based __attribute__((unused)),
                               uint32_t f_ig_eth_type_based __attribute__((unused)),
                               uint32_t f_ig_vlanid_based __attribute__((unused)),
                               uint32_t f_ig_port_based __attribute__((unused)),
                               uint32_t f_eg_out_vlan_insert __attribute__((unused)),
                               uint32_t f_eg_out_vlan_remove __attribute__((unused)),
                               uint32_t f_ig_out_vlan_aware __attribute__((unused)))
{
}

void get_bridge_if_vlan_config(uint32_t if_no __attribute__((unused)),
                               uint32_t *f_eg_vlan_insert __attribute__((unused)),
                               uint32_t *f_eg_vlan_remove __attribute__((unused)),
                               uint32_t *f_ig_vlan_aware __attribute__((unused)),
                               uint32_t *f_ig_src_ip_based __attribute__((unused)),
...
int32_t add_vlan_map(uint32_t ig_criteria_type __attribute__((unused)),
                     uint32_t ig_criteria __attribute__((unused)),
                     uint32_t new_vci __attribute__((unused)),
                     uint32_t dest_qos __attribute__((unused)),
                     uint32_t outer_vlan_ix __attribute__((unused)),
                     uint32_t in_out_etag_ctrl __attribute__((unused)),
                     uint32_t vlan_port_map __attribute__((unused)))
{
    return IFX_EPERM;
}

void del_vlan_map(uint32_t ig_criteria_type __attribute__((unused)),
                  uint32_t ig_criteria __attribute__((unused)))
{
}

int32_t get_vlan_map(uint32_t ig_criteria_type __attribute__((unused)),
                          ...
                          uint32_t *p_entry)
{
    ...
    ppe_lock_get(&g_ipv6_ip_lock, ppe_lock_flags);
    ...
    ppe_lock_release(&g_ipv6_ip_lock, ppe_lock_flags);
...
void del_ipv6_ip_entry(uint32_t entry) {
    ...
    if ( ( GEN_MODE_CFG1->ipv6_acc_en != 0 ) && ( x < MAX_IPV6_IP_ENTRIES_BLOCK ) ) {
        ppe_lock_get(&g_ipv6_ip_lock, ppe_lock_flags);
        BUG_ON( entry >= MAX_IPV6_IP_ENTRIES );
        ...
        for ( i = GEN_MODE_CFG1->ipv6_rout_num - 1; i >= 0; i-- ){
            for ( x = 0; x < MAX_IPV6_IP_ENTRIES_BLOCK; x++ ) {
                if (i < 0 || i >= MAX_IPV6_IP_ENTRIES_PER_BLOCK ){
                    printk(KERN_ERR "[%s] invalid entry: %d\n", __func__, i);
...
{ /*hash_table_id: note, in GRX500, LAN/WAN will share same hash table, so we add this variable for it */
    ...
    dbg("hash = %#x, src_port=%#x, dst_port=%#x, src_ip=%#x, dst_ip=%#x, f_is_lan=%d\n",
            hash,
            hash_info->src_port,
            hash_info->dst_port,
            hash_info->src_ip,
    ...
    dbg("hash = %#x, src_port=%#x, dst_port=%#x, src_ip=%#x, dst_ip=%#x, table=%d\n",
            hash,
            hash_info->src_port,
            hash_info->dst_port,
            hash_info->src_ip,
...
/*!
  ...
  \param activate_at_once       1: activate routing entry
                          ...
                          uint8_t *collion_flag)
{
    ...
    ppe_lock_get(p_lock, ppe_lock_flags);
    ...
    if ( occupation == (uint16_t)~0 )
    {
        ...
        //  no empty entry
        ppe_lock_release(p_lock, ppe_lock_flags);
        return IFX_EAGAIN;
...
ADD_ROUTING_ENTRY_GOON:
    ...
    dbg("%s, hash = %d, entry = %d, pcomare = 0x%08x, paction = 0x%08x, col=%d\n", f_is_lan ? "LAN" : "WAN", hash, entry, (unsigned int)pcompare, (unsigned int)paction, f_collision);
...
#if 0
void dump_wan_routing_entry(uint32_t entry){
    unsigned long ppe_lock_flags;
        volatile struct rout_forward_action_tbl *paction;
        volatile struct rout_forward_compare_tbl *pcompare;

        volatile struct rout_forward_action_tbl tmp_paction;
        volatile struct rout_forward_compare_tbl tmp_pcompare;
        ...
        ppe_lock_get(p_lock, ppe_lock_flags);
        memcpy(&tmp_paction, paction, sizeof(struct rout_forward_action_tbl));
        memcpy(&tmp_pcompare, pcompare, sizeof(struct rout_forward_compare_tbl));
        ppe_lock_release(p_lock, ppe_lock_flags);

        printk(KERN_ERR "e[%lu] active: %d \n", entry, (int) tmp_paction.entry_vld );
        printk(KERN_ERR "e[%lu] src_ip %#x \n", entry, (unsigned int)tmp_pcompare.src_ip );
        printk(KERN_ERR "e[%lu] dst_ip %#x \n", entry, (unsigned int) tmp_pcompare.dest_ip );
        printk(KERN_ERR "e[%lu] dst_port %x \n", entry, (unsigned int) tmp_pcompare.src_port );
        printk(KERN_ERR "e[%lu] src_port %x \n", entry, (unsigned int) tmp_pcompare.dest_port );
}
EXPORT_SYMBOL( dump_wan_routing_entry );
#endif

int32_t activate_routing_entry(uint32_t entry){
    unsigned long ppe_lock_flags;
    PPE_LOCK *p_lock;
    volatile struct rout_forward_action_tbl *paction;
    if ( (entry & 0x80000000) )
    {
        ...
        if ( entry >= MAX_ROUTING_ENTRIES_PER_HASH_BLOCK * MAX_HASH_BLOCK )
        {
            uint32_t collision_entry = entry - MAX_ROUTING_ENTRIES_PER_HASH_BLOCK * MAX_HASH_BLOCK;
            BUG_ON(collision_entry >= MAX_COLLISION_ROUTING_ENTRIES);
            paction  = ROUT_LAN_COLL_ACT_TBL( collision_entry );
        }
        else
        {
            paction  = ROUT_LAN_HASH_ACT_TBL(entry);
    ...
    else
    {
        ...
        if ( entry >= MAX_ROUTING_ENTRIES_PER_HASH_BLOCK * MAX_HASH_BLOCK )
        {
            uint32_t collision_entry = entry - MAX_ROUTING_ENTRIES_PER_HASH_BLOCK * MAX_HASH_BLOCK;
            BUG_ON(collision_entry >= MAX_COLLISION_ROUTING_ENTRIES);
            paction  = ROUT_WAN_COLL_ACT_TBL( collision_entry );
        }
        else
        {
            paction  = ROUT_WAN_HASH_ACT_TBL(entry);
...
void del_routing_entry(uint32_t entry)
{
    ...
    if ( (entry & 0x80000000) )
    {
        ...
        if ( entry >= MAX_ROUTING_ENTRIES_PER_HASH_BLOCK * MAX_HASH_BLOCK )
        {
            uint32_t collision_entry = entry - MAX_ROUTING_ENTRIES_PER_HASH_BLOCK * MAX_HASH_BLOCK;
            BUG_ON(collision_entry >= MAX_COLLISION_ROUTING_ENTRIES);
            pcompare = ROUT_LAN_COLL_CMP_TBL( collision_entry );
            paction  = ROUT_LAN_COLL_ACT_TBL( collision_entry );
        }
        else
        {
    ...
    else
    {
        ...
        if ( entry >= MAX_ROUTING_ENTRIES_PER_HASH_BLOCK * MAX_HASH_BLOCK )
        {
            uint32_t collision_entry = entry - MAX_ROUTING_ENTRIES_PER_HASH_BLOCK * MAX_HASH_BLOCK;
            BUG_ON(collision_entry >= MAX_COLLISION_ROUTING_ENTRIES);
            pcompare = ROUT_WAN_COLL_CMP_TBL( collision_entry );
            paction  = ROUT_WAN_COLL_ACT_TBL( collision_entry );
        }
        else
        {
                        ...
                        uint32_t *p_entry)
{
    ...
    ppe_lock_get(&g_capwap_lock, ppe_lock_flags);
...
;
    ...
    ppe_lock_release(&g_capwap_lock, ppe_lock_flags);
...
void get_capwap_mib(uint32_t entry, uint32_t *ds_mib, uint32_t *us_mib)
{
    ...
    if ( entry < g_capwap_entries ) {
        ...
        volatile struct capwap_config_tbl *p;

        ppe_lock_get(&g_capwap_lock, ppe_lock_flags);

        p = CAPWAP_CONFIG_TBL(entry);
                         ...
                         uint32_t *p_entry)
{
    ...
    ppe_lock_get(&g_wan_mc_lock, ppe_lock_flags);

    for ( i = 0; i < entries; i++ ) {
        if ( g_wan_mc_entry_occupation[i] != ~0U )
            goto ADD_WAN_MC_ENTRY_GOON;
    }
    //  no empty entry
    ppe_lock_release(&g_wan_mc_lock, ppe_lock_flags);
    return IFX_EAGAIN;
...
ADD_WAN_MC_ENTRY_GOON:
    ...
    BUG_ON(entry >= MAX_WAN_MC_ENTRIES);
...
int32_t add_bridging_entry(uint32_t port,
                           ...
                           uint32_t f_src_mac_drop __attribute__((unused)),
                           uint32_t dslwan_qid __attribute__((unused)),
                           uint32_t dest_list __attribute__((unused)),
                           uint32_t *p_entry)
{
    unsigned long ppe_lock_flags;
    ...
    *p_entry = ~0;
    ...
    ppe_lock_get(&g_mac_tbl_lock, ppe_lock_flags);
    ...
    for ( p_search_item = g_mac_tbl_hash[idx]; p_search_item; p_search_item = p_search_item->next )
        ...
        {
            p_search_item->ref++;
            ppe_lock_release(&g_mac_tbl_lock, ppe_lock_flags);
...
\*------------------------------------------------------------------------------------------*/
/*!
  \fn add_avm_6rd_tunnel_entry(struct net_device *dev,
                                        uint32_t *tunnel_idx)
  \ingroup AMAZON_S_PPA_PPE_D5_HAL_GLOBAL_FUNCTIONS
  \brief add tunnel ID
...
uint32_t add_6rd_tunnel_entry(uint32_t *tunnel_idx, __be32 saddr, __be32 daddr ) {
    unsigned long ppe_lock_flags;
    uint32_t entry;
    ...
    ppe_lock_get(&g_6rd_tunnel_lock, ppe_lock_flags);
    ...
    if(entry >= MAX_6RD_TUNNEL_ENTRIES){
        ...
        add_tunnel_6rd_header( (struct iphdr *)TUNNEL_6RD_TBL(entry), saddr, daddr);
    }
...
ADD_TUNNEL_ENTRY_GOON:
    ...
    *tunnel_idx = entry;
    ppe_lock_release(&g_6rd_tunnel_lock, ppe_lock_flags);
...
/*!
  \fn add_tunnel_6rd_header(volatile struct iphdr *iph, __be32 saddr, __be32 daddr )
  \ingroup AMAZON_S_PPA_PPE_D5_HAL_GLOBAL_FUNCTIONS
...
/*
 * avm reworked this interface
 */
uint32_t add_dslite_tunnel_entry(uint32_t *tunnel_idx,
				 ...
				 struct in6_addr *daddr)
{
	...
	dbg("try to setup dslite tunnel: src=%pI6, dst=%pI6\n", saddr, daddr);
	ppe_lock_get(&g_dslite_tunnel_lock, ppe_lock_flags);
	...
	for(entry = 0; entry < MAX_DSLITE_TUNNEL_ENTRIES; entry ++){
		if(!g_dslite_tunnel_counter[entry] && empty_entry >= MAX_DSLITE_TUNNEL_ENTRIES){
			...
			if(ipv6_addr_cmp((struct in6_addr *) &p_ip6hdr->saddr, saddr) == 0
			   && ipv6_addr_cmp((struct in6_addr *) &p_ip6hdr->daddr, daddr)== 0){
				goto ADD_TUNNEL_ENTRY_GOON;
			}
		}
	}
	...
	if(entry >= MAX_DSLITE_TUNNEL_ENTRIES){
		...
		ipv6_addr_copy(&ip6_hdr.saddr, saddr);
		ipv6_addr_copy(&ip6_hdr.daddr, daddr);
		*p_ip6hdr = ip6_hdr;
	}
...
ADD_TUNNEL_ENTRY_GOON:
	...
	ASSERT(tunnel_idx != NULL, "tunnel_idx == NULL");
	*tunnel_idx = entry;
	ppe_lock_release(&g_dslite_tunnel_lock, ppe_lock_flags);
                        ...
                        uint32_t *p_entry)
{
    ...
    ppe_lock_get(&g_pppoe_lock, ppe_lock_flags);
    ...
    {
        ppe_lock_release(&g_pppoe_lock, ppe_lock_flags);
        return IFX_EAGAIN;
    }
                      ...
                      uint32_t *p_entry)
{
    ...
    ppe_lock_get(&g_mtu_lock, ppe_lock_flags);
    ...
    {
        ppe_lock_release(&g_mtu_lock, ppe_lock_flags);
        return IFX_EAGAIN;
    }
...
uint32_t get_routing_entry_bytes(uint32_t entry, uint32_t reset_flag)
{
    ...
    if ( (entry & 0x80000000) )
    {
        p_lock = &g_lan_routing_lock;
        entry &= 0x7FFFFFFF;
        BUG_ON( entry >= MAX_LAN_ROUTING_ENTRIES );
        // p_occupation = g_lan_routing_entry_occupation;
        if ( entry >= MAX_ROUTING_ENTRIES_PER_HASH_BLOCK * MAX_HASH_BLOCK )
        {
            paction  = ROUT_LAN_COLL_ACT_TBL(entry - MAX_ROUTING_ENTRIES_PER_HASH_BLOCK * MAX_HASH_BLOCK);
                      ...
                      uint32_t *p_entry)
{
    ...
    ppe_lock_get(&g_mac_lock, ppe_lock_flags);
    ...
    if ( empty_entry < 0 )
    {
        ppe_lock_release(&g_mac_lock, ppe_lock_flags);
        return IFX_EAGAIN;
    }
                             ...
                             uint32_t *p_entry)
{
    ...
    ppe_lock_get(&g_outer_vlan_lock, ppe_lock_flags);
    ...
    {
        ppe_lock_release(&g_outer_vlan_lock, ppe_lock_flags);
        return IFX_EAGAIN;
    }
...
*/
uint8_t ifx_ppe_drv_multifield_control(uint8_t enable, uint32_t flag __attribute__((unused)))
{
...
/*!
    ...
    \return uint8_t, The return value can be any one of the following:  \n
               ...
               IFX_FAILURE on error
*/
uint8_t ifx_ppe_drv_get_multifield_status(uint32_t *enable, uint32_t flag __attribute__((unused)))
{
...
*/
int32_t ifx_ppe_drv_add_multifield_entry(PPA_MULTIFIELD_FLOW_INFO *p_multifield_info, int32_t *index, uint32_t flag __attribute__((unused)))
{
    unsigned long ppe_lock_flags;
    int ret=IFX_FAILURE;
    int32_t entry, empty_entry = -1;
    ...
    ppe_lock_get(&g_classification_lock, ppe_lock_flags);
    ...
    if ( empty_entry < 0 )
    {
        ppe_lock_release(&g_classification_lock, ppe_lock_flags);
        ret = IFX_EAGAIN;
...
/*!
    ...
    \return int32_t, return the bytes in the compare table. It can be any one of the following now: \n
               ...
               IFX_FAILURE on error
*/
int32_t ifx_ppe_drv_get_multifield_entry(int32_t index, PPA_MULTIFIELD_FLOW_INFO *p_multifield_info, uint32_t flag __attribute__((unused)))
{
    unsigned long ppe_lock_flags;
    int ret=IFX_SUCCESS;
    uint32_t compare[4] = {0};
...
*/
int32_t ifx_ppe_drv_del_multifield_entry_via_index(int32_t index, uint32_t flag __attribute__((unused)))
{
    unsigned long ppe_lock_flags;
    if ( index >= 0 && index < MAX_CLASSIFICATION_ENTRIES )
    {
...
{
    if (( (entry & 0x80000000) && ((entry & 0x7FFFFFFF) < MAX_LAN_ROUTING_ENTRIES ))
            || ( !(entry & 0x80000000) && ( (entry & 0x7FFFFFFF) < MAX_WAN_ROUTING_ENTRIES ) )) {
        uint32_t ret;
        volatile u32 *phit;
        u32 hitbit;
        ...
        if ( (entry & 0x80000000) )
        {
            ...
            if ( entry >= MAX_ROUTING_ENTRIES_PER_HASH_BLOCK * MAX_HASH_BLOCK )
            {
                ...
                BUG_ON(collision_entry >= MAX_COLLISION_ROUTING_ENTRIES);
                phit = ROUT_LAN_COLL_HIT_STAT_TBL(( collision_entry ) >> 5);
                hitbit = 1 << (BITSIZEOF_UINT32 - 1 - (collision_entry & 0x1F));
            }
            else
            {
                phit = ROUT_LAN_HASH_HIT_STAT_TBL(entry >> 5);
        ...
        else
        {
            ...
            if ( entry >= MAX_ROUTING_ENTRIES_PER_HASH_BLOCK * MAX_HASH_BLOCK )
            {
                ...
                phit = ROUT_WAN_COLL_HIT_STAT_TBL(collision_entry >> 5);
                hitbit = 1 << (BITSIZEOF_UINT32 - 1 - (collision_entry & 0x1F));
            }
            else
            {
...
uint32_t test_and_clear_bridging_hit_stat(uint32_t entry __attribute__((unused)))
{
    return 0;
}

uint32_t test_and_clear_bridging_hit_stat_batch(uint32_t entry __attribute__((unused)))
{
    return 0;
}

int32_t get_max_vfilter_entries(uint32_t vfilter_type __attribute__((unused)))
{
    return 0;
}
...
*/
int32_t get_qos_qnum( uint32_t portid, uint32_t flag __attribute__((unused)))
{
...
/*!
    ...
    \return returns the queue number supported on this port.
*/
int32_t get_qos_mib(uint32_t portid, uint32_t queueid, PPA_QOS_MIB *mib, uint32_t flag __attribute__((unused)))
{
    ...
    BUG_ON( queueid >= MAX_WAN_QOS_QUEUES );
...
/*!
    ...
    \return The return value can be any one of the following:  \n
               ...
               - IFX_FAILURE on error \n
*/
int32_t set_ctrl_qos_rate( uint32_t portid, uint32_t enable, uint32_t overhd_bytes, uint32_t flag) {
    ...
    if ( enable )
    {
        ...
        if (flag & PPE_QOS_ENABLE_CFG_FLAG_SETUP_OVERHD_BYTES)
            tx_qos_cfg.overhd_bytes = overhd_bytes;
        else
...
/*!
    ...
    \return The return value can be any one of the following:  \n
               ...
               - IFX_FAILURE on error \n
*/
int32_t get_ctrl_qos_rate( uint32_t portid, uint32_t *enable, uint32_t *overhd_bytes, uint32_t flag __attribute__((unused)))
{
    if( set_qos_port_id() != IFX_SUCCESS )  return IFX_FAILURE; //QOS Note: Different condition with different mode/platfrom
    ...
    if ( overhd_bytes ){
        *overhd_bytes = TX_QOS_CFG->overhd_bytes;
    }
...
/*!
    ...
    \return The return value can be any one of the following:  \n
               ...
               - IFX_FAILURE on error \n
*/
int32_t set_qos_rate( uint32_t portid, uint32_t queueid, uint32_t rate, uint32_t burst, uint32_t flag __attribute__((unused)))
{
    unsigned int i;
...
/*!
    ...
    \return The return value can be any one of the following:  \n
               ...
               - IFX_FAILURE on error \n
*/
int32_t get_qos_rate( uint32_t portid, uint32_t queueid, uint32_t *rate, uint32_t *burst, uint32_t flag __attribute__((unused)))
{
    struct wtx_eg_q_shaping_cfg qos_cfg = {0};
    ...
    if ( queueid >= TX_QOS_CFG->eth1_eg_qnum )
    {
        if( qos_queue_portid == PPA_WAN_QOS_PTM0) {
            queueid = ~queueid;
            BUG_ON( queueid >= MAX_PTM_QOS_PORT_NUM );
...
/*!
    ...
    \return The return value can be any one of the following:  \n
               ...
               - IFX_FAILURE on error \n
*/
int32_t set_ctrl_qos_wfq( uint32_t portid, uint32_t enable, uint32_t overhd_bytes, uint32_t flag) {
...
/*!
    ...
    \return The return value can be any one of the following:  \n
               ...
               - IFX_FAILURE on error \n
*/
int32_t get_ctrl_qos_wfq( uint32_t portid, uint32_t *enable, uint32_t *overhd_bytes, uint32_t flag __attribute__((unused)))
{
    if( set_qos_port_id() != IFX_SUCCESS ) return IFX_FAILURE; //QOS Note: Different condition with different mode/platfrom
...
/*!
    ...
    \return The return value can be any one of the following:  \n
               ...
               - IFX_FAILURE on error \n
*/
int32_t set_qos_wfq( uint32_t portid, uint32_t queueid, uint32_t weight_level, uint32_t flag __attribute__((unused)))
{
    struct wtx_eg_q_shaping_cfg qos_cfg = {0};
...
/*!
    ...
    \return The return value can be any one of the following:  \n
               ...
               - IFX_FAILURE on error \n
*/
int32_t get_qos_wfq( uint32_t portid, uint32_t queueid, uint32_t *weight_level, uint32_t flag __attribute__((unused)))
{
    struct wtx_eg_q_shaping_cfg qos_cfg = {0};
...
int32_t ifx_ppa_drv_get_mib(PPA_DSL_QUEUE_MIB *buffer __attribute__((unused)), uint32_t flag __attribute__((unused)))
{
    return -1;
}

int32_t ifx_ppa_drv_clear_mib(PPA_DSL_QUEUE_MIB *buffer __attribute__((unused)), uint32_t flag __attribute__((unused)))
{
    return -1;
}
...
static INLINE uint32_t set_wanitf(PPE_WANITF_CFG *wanitf_cfg, uint32_t flag __attribute__((unused)))
{
...
static int32_t set_variable_value(PPA_CMD_VARIABLE_VALUE_INFO *cfg, int32_t flag __attribute__((unused)) )
{
...
static int32_t get_variable_value(PPA_CMD_VARIABLE_VALUE_INFO *cfg, int32_t flag __attribute__((unused)) )
{
    int32_t res = IFX_SUCCESS;
...
//Fix warning message when exports API from different PPE FW Driver--begin
   ...
   case PPA_GENERIC_HAL_SET_BRDG_CFG:
    ...
    case PPA_GENERIC_HAL_GET_BRDG_VLAN_FITLER:
     ...
     case PPA_GENERIC_HAL_ACTIVATE_ROUTE_ENTRY:
     {
         PPE_ROUTING_INFO *route=(PPE_ROUTING_INFO *)buffer;
         return activate_routing_entry( route->entry );
     }
     case PPA_GENERIC_HAL_ADD_ROUTE_ENTRY:
        {
            ...
            {  //need to check dslite later ????
                return add_routing_entry(route->f_is_lan,  route->src_ip.ip.ip, route->src_port, route->dst_ip.ip.ip, route->dst_port,
                          ...
                          route->dslwan_qid,  route->dest_list, route->tnnl_info.tunnel_idx, route->active, &route->entry, &route->collision_flag);
            }
   ...
   case PPA_GENERIC_HAL_DEL_BR_MAC_BRIDGING_ENTRY:
    ...
    case PPA_GENERIC_HAL_ADD_6RD_TUNNEL_ENTRY:
        {
            ...
            return add_6rd_tunnel_entry( &tnnl_info->tunnel_idx,
					 tnnl_info->src.ip,
					 tnnl_info->dst.ip);
        }
	...
	{
	    PPE_TUNNEL_INFO *tnnl_info = (PPE_TUNNEL_INFO *)buffer;
            return add_dslite_tunnel_entry(&tnnl_info->tunnel_idx,
					   (struct in6_addr *)tnnl_info->src.ip6,
					   (struct in6_addr *)tnnl_info->dst.ip6);
        }
    ...
    case PPA_GENERIC_HAL_GET_ROUTE_ACC_BYTES:
        {
            /*
             * AVM-Concept: We never clear the MIB in HW
             * and calculate the Wrap-Around in Mini- Session Manager
             */
            PPE_ROUTING_INFO *route=(PPE_ROUTING_INFO *)buffer;
            route->mib_count = get_routing_entry_bytes( route->entry, 0);
            route->mib_factor = SESSION_MIB_MULTIPLEXER;
            route->mib_max = (1 << SESSION_MIB_COUNT_LEN ) - 1;
            return IFX_SUCCESS;
        }
    ...
    case PPA_GENERIC_HAL_SET_QOS_WFQ_CTRL:  //enable/disable WFQ
        {
            ...
            return set_ctrl_qos_wfq( enable_cfg->portid, enable_cfg->f_enable, enable_cfg->overhd_bytes, enable_cfg->flag );
        }
    case PPA_GENERIC_HAL_GET_QOS_WFQ_CTRL:  //get  WFQ status: enabeld/disabled
        {
            PPE_QOS_ENABLE_CFG *enable_cfg=(PPE_QOS_ENABLE_CFG *)buffer;
            return get_ctrl_qos_wfq( enable_cfg->portid, &enable_cfg->f_enable, &enable_cfg->overhd_bytes, enable_cfg->flag );
        }
    ...
    case PPA_GENERIC_HAL_SET_QOS_RATE_SHAPING_CTRL:  //enable/disable Rate shaping
        {
            PPE_QOS_ENABLE_CFG *enable_cfg=(PPE_QOS_ENABLE_CFG *)buffer;
            return set_ctrl_qos_rate( enable_cfg->portid, enable_cfg->f_enable, enable_cfg->overhd_bytes, enable_cfg->flag );
        }

    case PPA_GENERIC_HAL_GET_QOS_RATE_SHAPING_CTRL:  //get Rateshaping status: enabeld/disabled
        {
            PPE_QOS_ENABLE_CFG *enable_cfg=(PPE_QOS_ENABLE_CFG *)buffer;
            return get_ctrl_qos_rate( enable_cfg->portid, &enable_cfg->f_enable, &enable_cfg->overhd_bytes, enable_cfg->flag );
        }
