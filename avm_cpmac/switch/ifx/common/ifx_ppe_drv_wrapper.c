...
int (*ifx_ppa_drv_avm_get_atm_qid_hook)(struct atm_vcc *vcc, unsigned int prio) = NULL;
int (*ifx_ppa_drv_avm_get_ptm_qid_hook)(unsigned int skb_priority) = NULL;
int (*ifx_ppa_drv_avm_get_eth_qid_hook)(unsigned int mac_nr, unsigned int skb_priority) = NULL;
#else
int (*ifx_ppa_drv_get_dslwan_qid_with_vcc_hook)(struct atm_vcc *vcc, unsigned int skb_priority)= NULL;
...
int ifx_ppa_drv_avm_get_atm_qid(struct atm_vcc *vcc, unsigned int skb_priority){
    if (!ifx_ppa_drv_avm_get_atm_qid_hook) return 0;
    else return ifx_ppa_drv_avm_get_atm_qid_hook(vcc, skb_priority);
}

int ifx_ppa_drv_avm_get_ptm_qid( unsigned int skb_priority){
    if (!ifx_ppa_drv_avm_get_ptm_qid_hook) return 0;
    else return ifx_ppa_drv_avm_get_ptm_qid_hook(skb_priority);
}

int ifx_ppa_drv_avm_get_eth_qid(unsigned int mac_nr, unsigned int skb_priority){
    if (!ifx_ppa_drv_avm_get_eth_qid_hook) return 0;
    else return ifx_ppa_drv_avm_get_eth_qid_hook(mac_nr, skb_priority);
}

#else
int ifx_ppa_drv_get_dslwan_qid_with_vcc(struct atm_vcc *vcc, unsigned int prio)
{
    if( !ifx_ppa_drv_get_dslwan_qid_with_vcc_hook ) return 0;
    else return ifx_ppa_drv_get_dslwan_qid_with_vcc_hook(vcc, prio);
}
...
#if defined(CONFIG_IFX_PPA_AVM_USAGE)
EXPORT_SYMBOL(ifx_ppa_drv_avm_get_atm_qid_hook);
EXPORT_SYMBOL(ifx_ppa_drv_avm_get_ptm_qid_hook);
EXPORT_SYMBOL(ifx_ppa_drv_avm_get_eth_qid_hook);
#else
...
#if defined(CONFIG_IFX_PPA_AVM_USAGE)
EXPORT_SYMBOL(ifx_ppa_drv_avm_get_atm_qid);
EXPORT_SYMBOL(ifx_ppa_drv_avm_get_ptm_qid);
EXPORT_SYMBOL(ifx_ppa_drv_avm_get_eth_qid);
#else
...
uint32_t ifx_ppa_drv_activate_routing_entry (PPE_ROUTING_INFO *entry, uint32_t flag)
{
    if( !ifx_ppa_drv_hal_generic_hook ) return IFX_FAILURE;

    return ifx_ppa_drv_hal_generic_hook(PPA_GENERIC_HAL_ACTIVATE_ROUTE_ENTRY,(void *)entry, flag );
}
...
EXPORT_SYMBOL( ifx_ppa_drv_activate_routing_entry );
