...
#include <avm/net/ifx_ppa_api_common.h>
...
/*!
    AVM Concept allows ppe_clk_change with direct Argument
    Lantiq just allows to setup IFX_PPA_PWM_LEVELS
*/
#define FLAG_PPE_CLK_CHANGE_ARG_AVM_DIRCT (1 << 1)
#define MAX_PTM_SHAPED_PORTS 4
...
#define SESSION_MC_ENTRY                        0x80000000
...
#define PPE_QOS_ENABLE_CFG_FLAG_SETUP_OVERHD_BYTES (1 << 1)

typedef struct {
    uint32_t portid;
    uint32_t f_enable;
    uint32_t overhd_bytes; //AVMCCB: If you want to setup OVERHD_BYTES you need to set the flag:PPE_QOS_ENABLE_CFG_FLAG_SETUP_OVERHD_BYTES
    uint32_t flag;
} PPE_QOS_ENABLE_CFG;
...
typedef struct
{
    ...
    PPA_QOS_INTERVAL ptm_qos_port_rate_shaping[MAX_PTM_SHAPED_PORTS]; /*!< internal ptm qos port rate shaping parameters */
...
#ifdef __KERNEL__
#include <avm/net/ifx_ppa_hook.h>
#endif
